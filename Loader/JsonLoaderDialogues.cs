...
public class JsonLoaderDialogues : JsonLoader
{
    public JsonLoaderDialogues()
    {
        displayName = "dialogues"; // This accesses the inherited field
        path += "/JsonData/Quests/Dialogues/"; // Add to the streaming path
    }
    ...
    public async Task LoadFromJsonAsync(string fileName)
    {
        ...
        if (File.Exists(jsonPath))
        {
            ...
            if (dataWrapper != null)
            {
                ...
                {
                    foreach (Dialogue entry in dataWrapper.dialogues)
                    {
                        DialogueManager.Initialise(entry);
                        Repository.instance.dialogues.Add(entry);
                    }
